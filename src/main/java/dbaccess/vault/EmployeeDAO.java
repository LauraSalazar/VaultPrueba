...
public class EmployeeDAO extends GenericDAO {
	...
	public List<Employee> getEmpleadosByJobId(Integer id){
		 ...
		 String query = "select new model.Employee(c.firstName,c.lastName,c.email,c.phoneNumber,c.hireDate,c.job,c.salary,c.commissionPct,c.manager,c.department) from EMPLOYEES c join c.job j where j.id = :id";
