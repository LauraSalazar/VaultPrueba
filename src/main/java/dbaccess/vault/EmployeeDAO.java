...
public class EmployeeDAO extends GenericDAO {
	...
	public List<Employee> getEmpleadosByJobId(Integer id){
		 ...
		 String query = "select new model.Employee(c.firstName,c.lastName,c.email,c.phoneNumber,c.hireDate,c.job,c.salary,c.commissionPct,c.department,jh) from EMPLOYEES c join c.job j join c.jobHistories jh where j.id = :id";
